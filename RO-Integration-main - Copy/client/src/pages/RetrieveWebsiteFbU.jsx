...
function RetrieveWebsiteFbU() {
  ...
  const [selectedFeedbackStatus, setSelectedFeedbackStatus] = useState(null);
  ...
  const getWebsitefb = () => {
    http.get('/websitefb', { params: { userId: user.id } }).then((res) => {
      setWebsitefblist(res.data);
    });
  };

  const searchWebsitefb = () => {
    http.get('/websitefb', { params: { search, userId: user.id } }).then((res) => {
      setWebsitefblist(res.data);
    });
  };
  ...
  const handleMenuOpen = (event, id, status) => {
    ...
    setSelectedFeedbackStatus(status);
  };
  ...
  const columns = [
    ...
    { field: 'status', headerName: <span className="boldHeader">Status</span>, flex: 1 },
    {
      ...
      renderCell: (params) => (
        <IconButton
          onClick={(event) => handleMenuOpen(event, params.row.id, params.row.status)}
        >
          <MoreVert />
        </IconButton>
      )
  ...
  return (
    <Box style={{ paddingTop: "20px", width: "80vw" }}>
      ...
      >
        ...
        <MenuItem onClick={handleEditClick} disabled={selectedFeedbackStatus === 'Resolved'}>
          Edit
        </MenuItem>
