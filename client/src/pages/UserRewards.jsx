...
function UserRewards() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (user) {
          const redeemedResponse = await http.get(`/redeemed-rewards/${user.id}`);
          setRedeemedRewards(redeemedResponse.data || []);
        }

        if (userid) {
          const userResponse = await http.get(`/user/user-rewards/${userid}`);
  ...
  useEffect(() => {
    const pointsNeededToNextTier = {
      Bronze: 5000,
      Silver: 15000,
      Gold: 0,
    }[tier] || 0;

    const nextTierProgress = tier === "Gold" ? 100 : (points / pointsNeededToNextTier) * 100;
    setProgress(nextTierProgress > 100 ? 100 : nextTierProgress);
    setPointsNeeded(tier === "Gold" ? 0 : Math.max(pointsNeededToNextTier - points, 0));

    setCanPlay(tier === "Silver" || tier === "Gold");
      ...
      try {
        ...
        const eligible = response.data
          .filter((reward) => {
            ...
            if (tier === "Silver")
              return reward.tier_required === "Bronze" || reward.tier_required === "Silver";
            if (tier === "Gold")
              return (
          ...
          .filter((reward) => reward.points_needed <= points);
  ...
  const handleSpinResult = async (rewardPoints) => {
    setSpinnerLoading(true);
    try {
      const response = await http.put(`/user/user-rewards/${userid}`, {
        points: points + rewardPoints,
      });
      setPoints(prevPoints => prevPoints + rewardPoints); // Update the points state
  ...
  const handleUpdatePoints = async (newPoints) => {
    try {
      await http.put(`/user/user-rewards/${userid}`, { points: newPoints, tier: tier });
  ...
  const handlePurchaseReward = async () => {
    ...
    try {
      const redeemedReward = {
        ...
        tier: tier,
        timeRedeemed: new Date().toISOString(),
      };
  ...
  const searchRewards = async () => {
    try {
      const response = await http.get(`/reward?search=${search}`);
      setEligibleRewards(
        response.data.filter((reward) => {
          if (tier === "Bronze") return reward.tier_required === "Bronze";
          if (tier === "Silver")
            return reward.tier_required === "Bronze" || reward.tier_required === "Silver";
          if (tier === "Gold")
      ...
      toast.error("Failed to search rewards.");
    }
  };
