...
router.post("/", validateToken, async (req, res) => {
    ...
    try {
        ...
        const detailedErrors = error.errors ? error.errors.map(err => ({
            message: err.message,
            type: err.type,
            path: err.path,
            value: err.value,
    ...
    try {
        ...
        const bookings = await Booking.findAll({
            ...
            include: [
                ...
                { model: Event, as: "event", attributes: ["eventName", "eventDescription", "eventLocation", "eventStartDate", "eventEndDate", "eventStartTime", "eventEndTime", "eventOrganizerType", "eventOrganizerName", "eventScope", "eventActivity", "contactNumber", "email", "eventImage", "participationFee"] }
            ],
...
router.get("/user/:userId", validateToken, async (req, res) => {
    ...
    try {
            ...
            include: [
                {
                    ...
                    attributes: [
                        ...
                        "eventEndTime", "eventOrganizerType", 
                        "eventOrganizerName", "eventScope", "eventActivity", 
                        "contactNumber", "email", "eventImage"
                    ]
                }
            ],
            order: [["dateTimeBooked", "DESC"]]
        ...
        res.status(500).json({
            message: `Error fetching bookings: ${errorMessage}`,
            type: errorType,
            stack: errorStack,
        });
    }
